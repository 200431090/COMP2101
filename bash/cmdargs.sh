...
myargs=()
#Variables are created to check the status of the input 
vmode=0
dmode=0
...
while [ $# -gt 0 ]; do
  ...
  *)
  ...
  #Shifting the command line argument to process the next element
  shift
...
#If condition is used in this to check the status of the variable
if [ $vmode = 1 ]; then
...
#If condition is used in this to check the status of the variable
if [ $dmode -gt 0 ]; then
  echo "Debug Mode is On with debug level $dmode."
else
